...
<details>
  ...
  The following justification is nonsense, I just thought the idea of encoding data in recipe blogs was silly.
</details>
...
Just how _nona_ would have made it.

## how it works

<details>
    <summary>author's note</summary>
    To be a bit less verbose, we omit details like embedding a length header so the decoder knows how many bits to decode, using quantization so we're not operating on floats, and dealing with ambiguous re-tokenization when decoding
</details>
